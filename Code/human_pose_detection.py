...
def convert_xyxy_to_xywh(xyxy_bbox):
...
def parse_args():
    ...
    parser.add_argument('--stage', type=str, dest='stage', default='')
    parser.add_argument('--test_epoch', type=str, dest='test_epoch', default='')
...
# load pickle file
file_path = '/mnt/data/your_file_path.pkl'
...
for i in range(len(all_labels)):
    ...
    for j in range(len(all_labels[i])):
        ...
        if label == 'person':
            ...
            xyxy_bbox = all_bbs[i][j]
            bbox = convert_xyxy_to_xywh(xyxy_bbox)
            ...
            # save mesh (obj)
            save_obj(mesh_lixel_cam, face, f'../humans/{scene}/frame{i+1}_p{num_human}.obj')
            # save_obj(mesh_param_cam, face, f'output_mesh_param.obj')
